...
function lojas(q) {
  ...
  return [
    { nome:'Google Shopping', cor:'#4285F4', url:`https://www.google.com/search?tbm=shop&q=${t}` },
...
export default function BuscarPage() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-4">Encontre no Vai Lá e veja ofertas na web</p>

      <div className="sticky top-14 bg-gray-50 py-2 z-10">
      ...
      {!q && (
        <div className="mt-5">
          ...
          <div className="flex gap-2 flex-wrap">
            {SUGESTOES.map(s => (
              <button key={s} onClick={()=>setQ(s)} className="px-3 py-1.5 rounded-full bg-white border border-gray-200 text-sm hover:border-brand">{s}</button>
            ))}
          </div>
        </div>
      )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {lojas(q).map(l => (
                <a key={l.nome} href={l.url} target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ background:l.cor, color:l.fg||'#fff' }}>
                  {l.nome} →
                </a>
              ))}
            </div>
            <p className="text-[11px] text-gray-400 mt-1">Os links abrem a busca real de cada loja em nova aba.</p>
          </div>

          <div className="mt-6">
            <h2 className="font-bold mb-2">📍 No Vai Lá</h2>
            ...
            {!loading && done && items.length === 0 && (
              <div className="card p-5 text-center text-gray-500">
                Nenhum anúncio encontrado no Vai Lá. <Link href="/market/new" className="text-brand underline">Anuncie você mesmo!</Link>
              </div>
            )}
